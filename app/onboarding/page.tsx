...
export default function Onboarding() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    setError(null)
    try {
      // On garde le localStorage en backup si besoin
      ...
      if (!res.ok) {
        throw new Error(data.error || 'Erreur lors de la crÃ©ation du profil')
      }
      ...
      if (normalized.includes('rate limit') || normalized.includes('email rate')) {
        setCooldownRemaining(60)
        setError('Trop de demandes en peu de temps. Attendez 60 secondes avant de rÃ©essayer.')
      } else if (normalized.includes('smtp') || normalized.includes('email provider') || normalized.includes('sender')) {
        setError('L\u2019envoi de l\u2019email a Ã\u9chouÃ©. VÃ©rifiez la configuration SMTP et l\u2019adresse expÃ©diteur dans Supabase.')
      } else {
        setError(message)
      }
    } finally {
  ...
  if (isMagicLinkSent) return (
    <main className="min-h-screen bg-white flex flex-col items-center justify-center p-6 text-center">
      <div className="badge-new mb-6">Email envoyÃ©</div>
      <h1 className="display-title mb-6">VÃ©rifiez votre boÃ®te email</h1>
      <p className="hero-description max-w-md mx-auto mb-6">Un lien de connexion a Ã\u9tÃ© envoyÃ© Ã  <strong>{answers.email}</strong>.</p>
      <p className="hero-description max-w-md mx-auto mb-10">Cliquez sur ce lien pour terminer votre inscription et dÃ©couvrir votre programme personnalisÃ©.</p>
      <Link href="/" className="btn-dark">Retour Ã  l\u2019accueil</Link>
    </main>
  )
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <section className="hero-section photo-hero text-center" style={{ minHeight: 'auto', paddingBottom: '2rem' }}><div className="badge-new mx-auto">Bilan PersonnalisÃ©</div><h1 className="display-title mt-2" style={{ fontSize: 'var(--text-xl)' }}>Votre Profil</h1></section>
      <div className="onboarding-container" style={{ paddingTop: 0 }}>
        ...
        {step === 1 && <div className="step-content"><h1 className="display-title" style={{ fontSize: 'var(--text-lg)', marginBottom: '0.75rem' }}>Quel est votre objectif principal ?</h1><p className="hero-description mb-8" style={{ fontSize: 'var(--text-base)' }}>Up40 s\u2019adapte Ã  votre prioritÃ© du moment.</p><div className="options-grid">{[['force','\ud83d\udcaa','Gagner en force','Retrouver de vraies pompes complÃ¨tes.'],['sante','\ud83d\udee1\ufe0f','Confort articulaire','Pratiquer sans subir de gÃªnes aux articulations.'],['posture','\ud83e\uddcd','AmÃ©liorer ma posture','Ouvrir le torse et renforcer le haut du corps.']].map(([id, icon, title, description]) => <button key={id} type="button" className={`task-card ${answers.goal === id ? 'active' : ''}`} onClick={() => { handleSelect('goal', id); setTimeout(nextStep, 300) }}><div className="task-icon">{icon}</div><div className="task-details"><h4 style={{ margin: 0, fontSize: '1.1rem' }}>{title}</h4><p style={{ margin: 0, marginTop: '4px' }}>{description}</p></div></button>)}</div></div>}
        {step === 2 && <div className="step-content"><h1 className="display-title" style={{ fontSize: 'var(--text-lg)', marginBottom: '0.75rem' }}>Ressentez-vous des gÃªnes frÃ©quentes ?</h1><p className="hero-description mb-8" style={{ fontSize: 'var(--text-base)' }}>SÃ©lectionnez toutes les zones sensibles. Le programme s\u2019adaptera.</p><div className="options-grid mb-8">{[['epaules','Oui, aux Ã\u9paules'],['poignets','Oui, aux poignets'],['coudes','Oui, aux coudes'],['dos','Oui, au bas du dos'],['aucune','Aucune gÃªne']].map(([id, label]) => <button key={id} type="button" className={`task-card ${answers.pains.includes(id) ? 'active' : ''}`} onClick={() => togglePain(id)}><div className="task-details"><h4 style={{ margin: 0, fontSize: '1.1rem' }}>{label}</h4></div></button>)}</div><button className="btn-dark" style={{ width: '100%', padding: '1.25rem' }} onClick={nextStep} disabled={answers.pains.length === 0}>Continuer</button></div>}
        ...
        {step === 4 && <div className="step-content"><h1 className="display-title" style={{ fontSize: 'var(--text-lg)', marginBottom: '0.75rem' }}>DerniÃ¨re Ã\u9tape pour voir votre rÃ©sultat</h1><p className="hero-description mb-8" style={{ fontSize: 'var(--text-base)' }}>Nous crÃ©ons votre profil personnalisÃ©.</p><form onSubmit={handleSubmit} className="flex flex-col text-left" style={{ gap: '1.5rem' }}><div><label style={labelStyle}>Votre E-mail</label><input type="email" required style={inputStyle} placeholder="email@exemple.com" value={answers.email} onChange={event => handleSelect('email', event.target.value)} /></div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' }}>{[['age','Ã¢ge','Ans'],['height','Taille','cm'],['weight','Poids','kg']].map(([key, label, placeholder]) => <div key={key}><label style={labelStyle}>{label}</label><input type="number" required min={key === 'age' ? 18 : key === 'height' ? 140 : 40} max={key === 'age' ? 100 : key === 'height' ? 220 : 200} step={key === 'weight' ? '0.1' : '1'} style={inputStyle} placeholder={placeholder} value={answers[key as keyof Answers] as string} onChange={event => handleSelect(key as keyof Answers, event.target.value)} /></div>)}</div>{error && <div style={{ padding: '1rem', backgroundColor: '#fef2f2', color: '#dc2626', borderRadius: '0.75rem', fontSize: '0.9rem' }}>{error}{cooldownRemaining > 0 && <div style={{ marginTop: '0.5rem' }}>Nouvel essai possible dans {cooldownRemaining} s.</div>}</div>}<button type="submit" className="btn-dark" style={{ width: '100%', marginTop: '1rem', display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '1.25rem', fontSize: '1.1rem' }} disabled={isSubmitting || cooldownRemaining > 0 || !answers.email || !answers.age || !answers.weight || !answers.height}>{isSubmitting ? 'Envoi en cours...' : cooldownRemaining > 0 ? `RÃ©essayez dans ${cooldownRemaining} s` : 'Recevoir mon lien de connexion'}</button></form></div>}
      </div>
    </main>
  )
}
